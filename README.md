...
symbolami Unicode (emoji). Realizacja: **interpreter bajtkodowy** (kompilacja AST do bajtkodu + maszyna wirtualna)
(PLY = lex + yacc, Python).
...
**Rodzaj translatora.** Interpreter bajtkodowy. Program jest analizowany
...
| `ast_nodes.py` | węzły AST (czyste dane), środowisko zmiennych, błędy |
| `compiler.py` | kompilator: AST → bajtkod (lista instrukcji) |
| `vm.py` | maszyna wirtualna: wykonuje bajtkod na stosie |
...
Kompilator zamienia AST na bajtkod, VM wykonuje go na stosie. Wywołanie funkcji tworzy
